...
On macOS, declaring `.permissions = .{ "microphone" }` causes `native package` to write `NSMicrophoneUsageDescription` into the app bundle's `Info.plist`. The prompt says that the app's display name needs microphone access to capture audio. Apps that do not declare `microphone` ship no microphone privacy declaration.

The permission is necessary but not sufficient: the first audio-input preview is implemented only by the macOS system-WebView host. That host still asks macOS for consent at use time; macOS Chromium, Windows, Linux, iOS, and Android report the audio-input service as unsupported.
